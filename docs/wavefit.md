...
The model may be futher constrained by the following physical conditions:
...
1. Circular Polarization
  For circularly polarized electromagneticc waves, in addition to the above two conditions, we have further constraints on the amplitudes and phases:
  ```math
...
Assuming the window function is fixed (i.e., $x_0$, $y_0$, $\sigma$ are given), 
- the general model function has 14 free parameters: $k_x$, $k_y$, $f_i (i=1,\ldots,6)$, $\phi_i (i=1,\ldots,6)$.
- For the circularly polarized wave model, the number of free parameters reduces to 6: $k_x$, $k_y$, $E_w$, $B_w$, $\phi_E$, $\phi_B$.
